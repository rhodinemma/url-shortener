...
urlsRouter.get("/", async (ctx) => {
    ...
    ctx.response.body = await createShortURL(ctx.request.body as any, ctx.state.user_id)
}).put("/:id", async (ctx) => {
    ctx.response.body = await updateURL(ctx.params.id, ctx.request.body as any, ctx.state.user_id)
}).delete("/:id", async (ctx) => {
